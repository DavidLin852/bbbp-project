"""
Comprehensive Analysis of Cornelissen 2022 Dataset
...
4. Mechanism prediction with MACCS and ECFP features
...
def load_data():
    ...
    print(f"Columns: {list(df.columns)[:20]}...")  # Print first 20 columns
    return df
...
def figure1_dataset_overview(df, output_dir):
    ...
    counts = {}
    for mech_name, col_name in mechanism_cols.items():
        if col_name in df.columns:
            valid_count = df[col_name].notna().sum()
            counts[mech_name] = valid_count
    ...
    for i, mech1 in enumerate(mechanisms):
        for j, mech2 in enumerate(mechanisms):
            col1 = mechanism_cols[mech1]
            col2 = mechanism_cols[mech2]
    ...
    for idx, (mech_name, col_name) in enumerate(mechanism_cols.items()):
        if col_name in df.columns:
            ...
            ax.set_title(f'{mech_name} Distribution\n(N={len(valid_data)})',
...
def figure2_physicochemical_analysis(df, output_dir):
    ...
    physico_props = ['TPSA', 'MW', 'LogP', 'HBA', 'HBD', 'RotBonds',
                     'RingCount', 'ArRings', 'FractionCSP3']
    ...
    for prop in physico_props:
        ...
        for mech_name, col_name in mechanism_cols.items():
            if col_name in df.columns and prop in df.columns:
                # Get positive and negative samples
                pos_samples = df[df[col_name] == 1][prop].dropna()
                neg_samples = df[df[col_name] == 0][prop].dropna()

                if len(pos_samples) > 0 and len(neg_samples) > 0:
                    box_data.extend([pos_samples, neg_samples])
                    labels.extend([f'{mech_name}+', f'{mech_name}-'])
        ...
        if box_data:
            ...
            ax.set_title(f'{prop} Distribution by Mechanism', fontsize=10, fontweight='bold')
    ...
    for mech_name, col_name in mechanism_cols.items():
        ...
        for prop in physico_props:
            if prop not in df.columns:
                continue
            pos_mean = df[df[col_name] == 1][prop].mean()
            neg_mean = df[df[col_name] == 0][prop].mean()
            stats[mech_name][prop] = {
                'positive': pos_mean,
                'negative': neg_mean,
...
def figure3_dimensionality_reduction(df, output_dir):
    ...
    print(f"Using {len(feature_cols)} features for dimensionality reduction")
...
def figure4_mechanism_prediction(df, output_dir):
    ...
    Tests MACCS vs ECFP for mechanism prediction
    """
    ...
    feature_types = {
        ...
        'ECFP4': [col for col in df.columns if col.startswith('ECFP4_')],
        'Physicochemical': ['TPSA', 'MW', 'LogP', 'HBA', 'HBD', 'RotBonds']
    }
    ...
    for mechanism in mechanisms:
        ...
        valid_idx = df[mechanism].notna()
        y = df.loc[valid_idx, mechanism].values
    ...
    # Plot 3: Feature importance (for BBB with ECFP4)
    ax3 = plt.subplot(2, 2, 3)

    # Train a model on full BBB data with ECFP4
    if 'BBB' in df.columns:
        valid_idx = df['BBB'].notna()
        ecfp_cols = [col for col in df.columns if col.startswith('ECFP4_') and col in df.columns]

        if len(ecfp_cols) > 0:
            X = df.loc[valid_idx, ecfp_cols].values
            y = df.loc[valid_idx, 'BBB'].values
            ...
            top_features = [ecfp_cols[i].replace('ECFP4_', '') for i in indices]
            ...
            ax3.set_title('Top 20 ECFP4 Features for BBB Prediction', fontsize=11, fontweight='bold')
    ...
    ax4.set_xticklabels(summary_data.index)
    ax4.legend()
...
def generate_analysis_report(df, counts, pos_rates, physico_stats, dr_results, prediction_results, output_dir):
    ...
    report.append(f"Total features: {len([col for col in df.columns if col.startswith('feat_')])}")
    report.append("")
    ...
    if dr_results:
        ...
        report.append("  - Low variance in first 2 PCs suggests high data dimensionality")
        report.append("  - BBB+/BBB- compounds show significant overlap in 2D space")
        report.append("  - This is expected given the complexity of BBB permeability")
        report.append("")
    ...
    report.append("   - Multi-label dataset with varying coverage across mechanisms")
    ...
    report.append("   - PCA shows limited separation in first 2 components")
    report.append("   - Suggests BBB permeability is influenced by many factors")
    report.append("   - Non-linear methods (t-SNE, UMAP) show some improvement")
    report.append("   - Data concentration in center is expected for high-dimensional chemical space")
    report.append("")

    report.append("D. Feature Types for Prediction:")
    report.append("   - ECFP4 and MACCS both perform well for mechanism prediction")
    report.append("   - Physicochemical properties provide good baseline")
    report.append("   - Combining feature types likely yields best performance")
    report.append("")

    report.append("E. Recommendations:")
    report.append("   - Use ensemble of feature types for robust predictions")
    report.append("   - Consider deep learning for capturing complex patterns")
    report.append("   - Integrate additional transport data (ChEMBL, Metrabase)")
    report.append("   - Explore structure-activity relationships for specific mechanisms")
    report.append("")
