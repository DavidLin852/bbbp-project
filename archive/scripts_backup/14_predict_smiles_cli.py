...
from src.featurize.graph_pyg import BBBPredictGraphDataset, GraphBuildConfig
...
def main():
    ...
    ds_dummy = BBBPredictGraphDataset(root=str(tmp_root), df=df_dummy, cfg=gcfg)
    ...
    print("DEBUG requires_grad stats:")
    for name, p in model.named_parameters():
        if "backbone" in name:
            print(name, p.requires_grad)

    print("DEBUG ckpt keys:", state.keys())
    print("DEBUG model state dict first 5 keys:", list(model.state_dict().keys())[:5])
    print("DEBUG ckpt model first 5 keys:", list(state["model"].keys())[:5])

    total_norm = 0.0
    for p in model.parameters():
        total_norm += float(p.detach().abs().mean().item())
    print("DEBUG mean|param|:", total_norm / max(1, sum(1 for _ in model.parameters())))
    ...
    with torch.no_grad():
        for i in range(len(ds)):
            ...
            print("DEBUG logit shape:", tuple(logit.shape), "logit_scalar:", logit_scalar, "prob:", prob)
